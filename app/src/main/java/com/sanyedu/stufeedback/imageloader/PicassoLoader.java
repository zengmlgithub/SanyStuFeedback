package com.sanyedu.stufeedback.imageloader;
...
import com.sanyedu.sanylib.imageloader.BitmapCallBack;
import com.sanyedu.sanylib.imageloader.ILoaderStrategy;
...
public class PicassoLoader implements ILoaderStrategy {
	...
	private static LruCache sLruCache = new LruCache(StuFeedbackApp.gApp);

	private static Picasso getPicasso() {
		if (sPicassoSingleton == null) {
			synchronized (PicassoLoader.class) {
				if (sPicassoSingleton == null) {
					sPicassoSingleton = new Picasso.Builder(StuFeedbackApp.gApp).memoryCache(sLruCache).build();
				}
			}
		}
	...
	public void clearDiskCache() {
		File diskFile = new File(StuFeedbackApp.gApp.getCacheDir(), PICASSO_CACHE);
		if (diskFile.exists()) {
			//这边自行写删除代码
